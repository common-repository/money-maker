...
function money_maker_installed() {
...
add_action('plugins_loaded', 'money_maker_installed');
add_action('init', 'money_maker_installed');

/* Runs when plugin is activated */
function money_maker_activate() {
 	if( function_exists( 'paltipPluginInstalled' ) ) {
		trigger_error("You already have PalTip plugin installed. this plugin is doing the same, so there is no need for installing both of them.", E_USER_ERROR);
		wp_die("You already have PalTip plugin installed. this plugin is doing the same, so there is no need for installing both of them.",'already installed');
...
register_activation_hook(__FILE__,'money_maker_activate');

/* Runs on plugin deactivation*/
register_deactivation_hook( __FILE__, 'money_maker_remove' );
function money_maker_remove() {
...
function money_maker_admin_menu() {
	add_options_page(publicFunctions::$PLUGIN_NAME.' - settings', publicFunctions::$PLUGIN_NAME, 'manage_options','PalTip_options', 'money_maker_settings_html_page');
}

/**
 * Add Settings link to plugins - code from GD Star Ratings
 */
function add_money_maker_settings_link($links, $file) {
	static $this_plugin;
...
if ( is_admin() ){
	/* Call the html code */
	add_action('admin_menu', 'money_maker_admin_menu');
	add_filter('plugin_action_links', 'add_money_maker_settings_link', 10, 2 );
}

function money_maker_settings_html_page(){
...
function money_maker_update_links($content){
	$link_changer = new LinkChanger( publicFunctions::getPTID(), publicFunctions::getPTEmail() , publicFunctions::getPTUserName());
	return $link_changer->changeLinksInContent($content);
}
add_filter('content_save_pre','money_maker_update_links');

function money_maker_custom_dashboard_help() {
...
add_action('right_now_content_table_end', 'money_maker_custom_dashboard_help');
?>